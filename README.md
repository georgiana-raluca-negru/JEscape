...
4. **`View Leaderboard`**: Displays the top global escape scores.

**In-Game Commands:**
5. **`move <direction>`**: Navigates the player between connected locations.
6. **`inspect <target>`**: Examines rooms, items, or containers to reveal hidden objects or clues.
7. **`take <item>`**: Picks up an item from the room or from inside an unlocked container.
8. **`use <item> on <target>`**: Applies a tool/key to interact with lockable objects (doors, safes).
9. **`inventory`**: Displays all items currently carried by the player.
10. **`stats`**: Shows the active game session data (Time remaining, difficulty, current room).

---

## Domain Models and Object Types
The engine is built upon **more than 8 core domain classes**, fully encapsulated with `private` attributes, appropriate `getters/setters`, and overridden `toString()` methods for logging and debugging. The core object types include:

1. `PlayerProfile` (Holds user credentials, aesthetic titles, and personal history)
2. `GameSession` (Tracks the active state of a player's current run)
3. `GameResult` (Stores the historical data of a finished game)
4. `Location` (Represents the nodes/rooms of the game map)
5. `GameMap` (Aggregates locations and defines the starting point)
6. `DoorObject` (Manages transitions between locations and lock states)
7. `ContainerObject` (Lockable entities that hold other items)
8. `ToolItem` (Functional items used to interact with the environment)
9. `ClueItem` (Objects that provide textual hints to the player)

---

## Object-Oriented Architecture (Inheritance & Interfaces)
The project utilizes a deep, multi-level inheritance hierarchy to ensure code reusability and polymorphisism:
